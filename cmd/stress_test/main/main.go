...
func main() {
	...
	msgChan := make(chan []byte, 5000)
	connMutex := &sync.RWMutex{}
	receive.RegisterStressTestEventListener(redisService, wsService, log, connections, subsPool, &subsWG, listenerCtx, allowedReceivers, msgChan, connMutex).Run()
	...
	eventHandler := handlers.NewStressTestHandler(
		redisService,
		wsService,
		log,
		connections,
